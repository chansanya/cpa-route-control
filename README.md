...
- 后台刷新遇到临时网络错误时不会再退出当前页面，而是保留本地界面并自动重连。
- 重连从约 1.5 秒开始指数退避，最长间隔 30 秒；连接恢复后自动拉取最新配置。
- 重连期间侧栏显示 `CPA 重连中`，可点击重连图标立即重试，也可以使用退出按钮返回连接页。
- 如果用户存在未保存的权重或优先级修改，重连成功后会保留草稿，不会被后台配置覆盖。
- 设置或移除别名时，如果对应远程模型已经删除，本次写入会被拒绝，并自动刷新本地模型列表。
